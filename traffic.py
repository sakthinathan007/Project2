...
# ─────────────────────────────────────────────
# PREDICTION SYSTEM (REALISTIC NOISE)
# ─────────────────────────────────────────────
...
def get_route(row):
    ...
    if not routes or routes == "None":
        routes_list = ["Ring Road", "Bypass Road", "Highway", "Service Road"]
    else:
        routes_list = [r.strip() for r in routes.split(",")]

    return random.choice(routes_list)


# ─────────────────────────────────────────────
# OUTPUT SYSTEM (SMART LOGIC)
# ─────────────────────────────────────────────
def show_output(row):
    print("\n" + "="*50)
    ...
    # LIVE STATUS (Improved - dynamic)
    live_count = int(row["vehicle_count"]) + random.randint(-10, 10)
    live_speed = int(row["avg_speed_kmh"]) + random.randint(-3, 3)
    ...
    # Optional (extra smart)
    if row["event"] != "None":
        print(f"  Reason   : High traffic due to {row['event']}")
...
if __name__ == "__main__":
    random.seed()  # dynamic randomness
    main()